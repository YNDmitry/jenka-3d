...
export function useArcadeInteraction(
  ...
  // Start at 0 (Dark) to avoid pop, then flash up
  const lightMultiplier = ref(0.0)
  ...
  function swap() {
    ...
    tl.to([stateA.scale, stateB.scale], {
    ...
    tl.to([stateA.influence, stateB.influence], {
        value: (i: number) => (i === 0 ? targetA.infl : targetB.infl),
        duration: dur,
      }, 0)

    // Juice
    const isMovingToFront = (i: number) => (i === 0 ? !isSwapped.value : isSwapped.value)
    tl.to([offsetsA.pos, offsetsB.pos], { y: 0.4, duration: 0.3, ease: 'power2.out', yoyo: true, repeat: 1 }, 0)
    tl.to([offsetsA.rot, offsetsB.rot], { x: -0.15, duration: 0.2, ease: 'power2.in', yoyo: true, repeat: 1 }, 0)
    tl.to([offsetsA.rot, offsetsB.rot], {
    ...
    // Impact
    tl.to([offsetsA.pos, offsetsB.pos], { y: 0, duration: 0.15, ease: 'expo.in' }, dur - 0.15)
    tl.to([offsetsA.scale, offsetsB.scale], { y: -0.05, x: 0.02, z: 0.02, duration: 0.05, ease: 'power4.out' }, dur - 0.05)
    tl.to([offsetsA.scale, offsetsB.scale], { y: 0, x: 0, z: 0, duration: 0.4, ease: 'elastic.out(1, 0.3)' }, dur)

    // Cam & Light FX
    gsap.to(camParallaxOffset.value, { z: 1.5, duration: dur * 0.6, ease: 'power2.inOut' })
    gsap.to(camParallaxOffset.value, {
      ...
      ease: 'expo.in',
      onComplete: () => {
        gsap.to(camParallaxOffset.value, { z: 0, duration: 0.5, ease: 'elastic.out(1, 0.5)' })
      },
    })

    const flashDelay = dur - 0.1
    gsap.to(lightMultiplier, {
      value: 4.0, // Flash
      duration: 0.05,
      delay: flashDelay,
      yoyo: true,
      repeat: 1,
      onComplete: () => {
        gsap.to(lightMultiplier, { value: 1.0, duration: 0.3 })
      },
    })
  ...
  function handleHover(model: 'A' | 'B', over: boolean) {
    ...
    if (over) {
      ...
      if (isBack) {
        gsap.to(hoverState, { [model]: 0.25, duration: 0.5, ease: 'back.out(1.2)', overwrite: true, onUpdate: invalidate })
      }
    } else {
      document.body.style.cursor = 'auto'
      gsap.to(hoverState, { [model]: 0, duration: 0.4, ease: 'power3.out', overwrite: true, onUpdate: invalidate })
    }
  }

  function handleClick(e: any, model: 'A' | 'B') {
    if (gsap.isTweening(stateA.pos)) return
    const isFront = (model === 'A' && !isSwapped.value) || (model === 'B' && isSwapped.value)
    if (isFront) return
    e.stopPropagation()
  ...
  function onMouseMove(e: MouseEvent) {
    if (!active.value || reducedMotion.value || device.value !== 'desktop') return
  ...
  onBeforeRender(({ elapsed }) => {
    ...
    if (modelA.value) {
      ...
      modelA.value.scale.set(s.scale.x + (o.scale.x + hoverState.A) * signA, s.scale.y + o.scale.y + hoverState.A, s.scale.z + o.scale.z + hoverState.A)
    }
    ...
    if (modelB.value) {
      ...
      modelB.value.scale.set(s.scale.x + (o.scale.x + hoverState.B) * signB, s.scale.y + o.scale.y + hoverState.B, s.scale.z + o.scale.z + hoverState.B)
    }
  })

  function playEntryAnimation() {
    // 1. Initial State: Blinding Light + Submerged
    lightMultiplier.value = 0.0 // Very bright flash
    offsetsA.pos.y = 0 // Start low
    offsetsB.pos.y = 0
    
    // 2. Animate Light: 6.0 -> 1.0 (Stabilize)
    gsap.to(lightMultiplier, {
      value: 1.0,
      duration: 1.4,
      ease: 'power2.out',
    ...
    // 3. Animate Rise: -0.5 -> 0.0 (Rise up)
    gsap.to([offsetsA.pos, offsetsB.pos], {
      y: 0,
      duration: 1.2,
      ease: 'back.out(1.2)', // Overshoot slightly for life
      delay: 0.1
    })

    // 4. Subtle Camera Push (Optional, kept very small)
    camParallaxOffset.value.z = 0.5
    gsap.to(camParallaxOffset.value, {
      z: 0,
      duration: 1.5,
      ease: 'power3.out'
